<template>
  <view class="content pos-bot flex fd-c">
    <!--    <uni-nav-bar :title="topRow.merchantName"/>-->
    ...
    <view class="flex-1 of-a ">
      ...
      <z-paging width="100%" show-loading-more-when-reload ref="artList" refresher-threshold="100rpx"
        ...
        <swiper v-if="bannerList.length" class="scroll-view_H mt-44" :indicator-dots="true"
          ...
          <!--          <swiper-item>-->
          <!--            <image src="@/static/img/drop-img.svg" alt="" class="wd ta-c mx-4" v-if="isHaveDropId!=='0'"-->
          <!--                   mode="aspectFill" @click="onNavigateToPage('dropDetail',{merchantId:merchantId,id:isHaveDropId})"/>-->
          <!--          </swiper-item>-->
        </swiper>
        <view class="flex ai-c jc-c mt-68 mx-60">
          ...
          <view class="flex fd-c ai-c pos-rel" @click="barClick(4)">
...
export default {
  ...
  async onShow() {
    ...
    if (uni.getStorageSync('refresh')) {
      this.$refs.artList.reload()
    }
    await this.init();
  },
  components: {
    uniNavBar, uniBadge, ImageView, HchPoster, Login,UniNoticeBar
  },
  methods: {
    async getMerchant() {
